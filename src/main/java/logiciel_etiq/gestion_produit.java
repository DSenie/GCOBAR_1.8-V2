...
public class gestion_produit {
	...
	public void select_produit(String code_p ){
		...
		String query5="SELECT code_produit,designation,dimension,Format(date_lc,'dd/MM/yy'),Format(date_lancement,'dd/MM/yy'),type" +
				", f.code_fourniss,f.nom_fourniss , qte_produire,p.intitule,Ncontrat  " +
				" FROM produit,fournisseur f , profils_prod p where code_produit='"+code_p+"' and  f.code_fourniss=produit.fournisseur " +
				" and  p.Ncode=produit.code_profil ";
  ...
  public void update_produit(String code,String designation,String dimension,String fournisseur,
		 String  date_lc,String date_lancement,int qte_produire,String intitule_profil,String type,String code1,String Ncontrat){
		try {
			String select = "select Ncode from profils_prod where intitule='"+intitule_profil+"' ";
			String codeProfil=CConnect.Requete(select,bdd).get(0);

			System.out.println("eeeeee"+fournisseur);
			  String insert2 = "update produit set date_lc=#"+imp.date_traiter(date_lc)+"#,date_lancement=#"+imp.date_traiter(date_lancement)+"#"
			  		+ ",designation='"+designation+"' , dimension='"+dimension+"',"
			  				+ "fournisseur='"+fournisseur+"',qte_produire="+qte_produire+","
			  						+ "code_profil="+codeProfil+",type='"+type+"',code_produit='"+code1+"',Ncontrat='"+Ncontrat+"'"
  ...
  public void ajouter_produit(String code,String designation,String dimension,String fournisseur,
			 String  date_lc,String date_lancement,int qte_produire,String intitule_profil,String type,
			 String Ncontrat){
		try {
			String select = "select Ncode from profils_prod where intitule='"+intitule_profil+"' ";
			String codeProfil=CConnect.Requete(select,bdd).get(0);
    		...
    		String insert2 = "insert into produit"
    				...
    				+ ",qte_produire,code_profil,type,Ncontrat) values('"+code+"','"+designation+"','"+dimension+"','"+fournisseur+"',"
    						+ "#"+imp.date_traiter(date_lc)+"#, #"+imp.date_traiter(date_lancement)+"#, "+qte_produire+", "+codeProfil+",'"+type+"','"+Ncontrat+"')";
    		CConnect.Insert(insert2,bdd);
