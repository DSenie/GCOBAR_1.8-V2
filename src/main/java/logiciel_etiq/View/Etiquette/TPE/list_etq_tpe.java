...
public class list_etq_tpe extends generale {


    private Object [] entete={"Code/Designation Article","Chaine","Date","Code Enie","Serial Number","IMEI"};
    ...
    public list_etq_tpe(final String log){
    ...
    public void composant(String log){
        ...
        initFrame("Liste des Etiquette TPE", panel, this, log,"Liste des Etiquette TPE");
        ...
        recherche();
        rech_but.addActionListener(e ->   recherche());
                ...
                e -> {
                    if (dimension_comb.getSelectedIndex() == 0) {
                        JOptionPane.showMessageDialog(null, "Vous devez choisir un type d'étiquette"); }
                          ...
                          removeDoublant(tab,3);
                        if (selectedRow != 0) {
                            String parcour = constant.etiq_tv + dimension_comb.getSelectedItem() + tab.table.getSelectedRowCount() + "_" + ".pdf";
        ...
        exporter.addActionListener(
                e -> generateExcel("list_etq_tpe","Liste des etiquette TPE",  tab, entete,  selectedRow));
    ...
    ArrayList list_fiche=imp_tpe.select_etq_tpe(filterText.getText(),datedebutText.getDate(),datefinText.getDate());
    remplirTab(list_fiche, 6,  tab,0);
