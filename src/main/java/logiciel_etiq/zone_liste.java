...
public class zone_liste extends JFrame{
	/**
	 * 
	 */
...
//	private static String laf="com.sun.java.swing.plaf.nimbus.NimbusLookAndFeel";
	private  Object [] entete={"Code Zone","Intitule Zone"};
...
public void composant(){
	 selectioncomb.prv.add("zone_liste");
	...
	tab.table.getTableHeader().setBackground(Color.black);
	 ...
	 tab.setStyle(2);
	  ...
	  list_zone=   use.select_zone_code();
	    for(i=0;i<list_zone.size();i=i+2){tab.ajouter();}
		int j=1;
		int l = 0; i=1;
		while(l<list_zone.size()){
		
		 while(l<j*2){
			 tab.getTable().setValueAt(list_zone.get(l).toLowerCase(), j-1, i-1);
			 i=i+1; l=l+1;
		 }
		 i=1;
		j=j+1;		
    ...
    tab.table.setFont(police_fi);
	    ...
	    count = new JLabel("Le nombre des list_zones :  "+tab.table.getRowCount());
