...
public class Recherche_general extends JFrame {
	/**
	 *
	 */
	...
	Object [] entete={"Code/Designation","parcel","imei1","imei2","couleur"};
	final Tableau tab=new Tableau(entete);
	
	Object [] entetetpe={"Code/Designation","parcel","imei1","SN"};
	final Tableau tabtpe=new Tableau(entetetpe);
	...
	JMenuItem menuItemtpeid = new JMenuItem("Imprimer etiquette ID" );
	JMenuItem menuItemtpeimei = new JMenuItem("Imprimer etiquette IMEI" );
	...
	JScrollPane ptpe=new JScrollPane(tabtpe);
	...
	public Recherche_general (final String log){
...
//		}
		...
		tabtpe.table.setFont(police_fi);
	...
	//	pan2.add(pandatedebut);
        ...
        ptpe.setPreferredSize(new Dimension(700,700));
	...
	private void build() {
		...
		tabtpe.table.addMouseListener(new MouseAdapter() {
			@Override
	...
	//	popupMenu.add(menuItem);
		popupMenutpe.add(menuItemtpeid);
		popupMenutpe.add(menuItemtpeimei);

		//   popupMenu.add(menuItem2);

		menuItemtpeid.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						String model ="C:\\GCOBAR\\CODE\\etq_tpe_list_id.jrxml";
						impTpe(model);
					}});


		menuItemtpeimei.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						String model ="C:\\GCOBAR\\CODE\\etq_tpe_list.jrxml";
						impTpe(model);
					}});
					...
					public void actionPerformed(ActionEvent e) {
						...
						for (int i=0; i<z; i++) {
							if(!listrc.contains((String) tab.table.getValueAt(selection[i], 1))){
								String parcel=tab.table.getValueAt(selection[i], 1).toString().replace("/", "_");
								listrc.add(parcel);
								listrc1.add((String) tab.table.getValueAt(selection[i], 1));
							}
...
//		   		    		}
		  ...
		  menuItemtpeexcel.addActionListener(
					new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							att = new Thread(){
								public void run(){
									...
									if (reponse==JOptionPane.YES_OPTION){
										try {
											...
											//"Code/Designation","parcel","imei1","SN"
											...
											int[] selection = tabtpe.table.getSelectedRows() ;
											int z =selection.length;
											...
											for (int i=0; i<z; i++) {
												...
												for (int j=0;j<tabtpe.table.getColumnCount();j++) {
													cell = row.createCell(j);


														cell.setCellValue((String) tabtpe.table.getValueAt(selection[i], j));
	...
	private void showPopuptpe(MouseEvent ev) {
		if (ev.isPopupTrigger()) {
			Point ptpe = new Point(ev.getX(), ev.getY());
			tabtpe.table.columnAtPoint(ptpe);
			selectedRowtpe = tabtpe.table.rowAtPoint(ptpe);
			popupMenutpe.show(ev.getComponent(), ev.getX(), ev.getY());
  ...
  private void recherche(){
	  int rows = tab.table.getRowCount();
		...
		if(articlecombo.getSelectedItem()=="Portable"){
			for (int i = rows - 1; i >= 0; i--) {
		        System.out.println("i did rmove");
				((DefaultTableModel) tab.table.getModel()).removeRow(i);
			}
	
		
		list_rech=imp.recherche_portable(filtre, datedebut, datefin);
	...
	while(l1<list_rech.size()){
		...
		j1=j1+1;
	      ...
	      count.setText("Nombre de Portable  :"+tab.table.getRowCount());
			 ...
			 int rowstpe = tabtpe.table.getRowCount();
			System.out.println("pfgggggggdfgg"+filterText.getText());
			//panel.remove(p);
			list_rech=imp.recherche_tpe(filtre, datedebut, datefin);
			

			for (int i = rowstpe - 1; i >= 0; i--) {
				((DefaultTableModel) tabtpe.table.getModel()).removeRow(i);
			}
		

			//list_rech=imp.select_emei_table(filterText.getText());
			for(int i1=0;i1<list_rech.size();i1=i1+5){ tabtpe.ajouter();}
			int j1=1;
			int l1 = 0; i1=1;
			while(l1<list_rech.size()){
				while(l1<j1*5){
					tabtpe.getTable().setValueAt(list_rech.get(l1).toLowerCase()+" "+list_rech.get(l1+1).toLowerCase(), j1-1, 0);
					tabtpe.getTable().setValueAt(list_rech.get(l1+2).toLowerCase(), j1-1, 1);
					tabtpe.getTable().setValueAt(list_rech.get(l1+3).toLowerCase(), j1-1, 2);
					tabtpe.getTable().setValueAt(list_rech.get(l1+4).toLowerCase(), j1-1, 3);
			...
			count.setText("Nombre de TPE  :"+tabtpe.table.getRowCount());
...
public void impTpe(String model){
	listrc.clear();
	int[] selection = tabtpe.table.getSelectedRows() ;
	int z =selection.length;
	for (int i=0; i<z; i++) {
		if(!listrc.contains((String) tabtpe.table.getValueAt(selection[i], 1))){
			String parcel=tabtpe.table.getValueAt(selection[i], 1).toString().replace("/", "_");
			listrc.add(parcel);
			listrc1.add((String) tabtpe.table.getValueAt(selection[i], 1));
		}
//		   		    		if(!listrc1.contains((String) tab.table.getValueAt(selection[i], 0))){
//
//		   		    	    listrc1.add((String) tab.table.getValueAt(selection[i], 0));
...
//		   		    		}
	...
	if(selectedRowtpe!=-1){
		...
		if(model.contains("id"))
		 parcour= "C:\\GCOBAR\\pdf\\etq_emballage\\"+listrc+"etq_emballage_id.pdf";
		else 		 parcour= "C:\\GCOBAR\\pdf\\etq_emballage\\"+listrc+"etq_emballage_iemi.pdf";
