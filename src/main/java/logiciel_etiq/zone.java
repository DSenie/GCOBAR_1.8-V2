...
//		}
		...
		valid_modif.addActionListener(new ActionListener() {
			...
			//	use.select_zone_jtext(designation.getText());
				...
				if (comb == true) {
					JOptionPane.showMessageDialog(null,
							"Veuillez d'abord Sélectionner une Zone \n");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (existe == false) {
					...
					if (desc == true) {
						...
						msg += "L'effectif de poste doit étre un nombre\n";
					} else if (repet == true) {
						msg += "Il existe un ou plusieur postes dupliqués \n";
					}
					if (!msg.equals("")) {
						JOptionPane.showMessageDialog(null, msg);
					}
