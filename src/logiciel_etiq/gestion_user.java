...
public class gestion_user {
	...
	// protected String bdd;
	 ...
	 public ArrayList<String>  selection_profils(String matri){
		  ...
		  String query = "SELECT profils FROM user where login='"+matri+"'";
		  ...
		  //  System.out.println(code);
		    String query2 = "SELECT poste,"
			 		+ "chaine"
			 		+ ",zone,personelle,user,code,code_ref,code_gener,article,type"
			 		...
			 		+ "FROM profils,user where user.profils=profils.code_prof  and user.profils="+code;
	 ...
	 public void  selection(String matri){
		    ...
		    String query = "SELECT matricule,nom,prenom,login,password,priorite,role,profils FROM user where matricule='"+matri+"'"; 
    ...
    if(CConnect.Requete(query, bdd).size()>1){
      JOptionPane.showMessageDialog(null,"Cette personne existe d�j�.");
        ...
        JOptionPane.showMessageDialog(null,""+nom+" "+prenom+" a �t� bien ajout�");
	     }  
	}
	
	
	public void setupdate(String matri,String nom,String prenom,String login,String password,String prio,String role){
		  String update= "update user set nom='"+nom.toLowerCase()+"', prenom='"+prenom.toLowerCase()+"', login='"+login+"', "
...
if (reponse== JOptionPane.YES_OPTION){
	...
	String update= "delete from user where matricule='"+matri+"'"; 
	  CConnect.Insert(update,bdd);
	  return true;
...
public boolean admin(String login){
	...
	try {
	    ...
	    ResultSet result = state.executeQuery("select priorite from user where login='"+login+"'");

	    while(result.next()){
	    	String prio= result.getString("priorite") ;
	    
	         if(prio.equals("1")) vrai=true;
	         else vrai=false;
...
public ArrayList<String> select(){
    ArrayList <String>resultat= new ArrayList<String>() ;
   	    String query5 = "SELECT matricule,nom,prenom,login,priorite,role FROM user where matricule<>'00000'"; 
...
if (reponse== JOptionPane.YES_OPTION){
	...
	CConnect.Insert(update,bdd);
	  ...
	  JOptionPane.showMessageDialog(null,"La zone a �t� bien supprim�e");
	  return true;
}
return false;}
