		...
		menuItemtpe.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						for (int i=0; i<z; i++) {
							if(!listrc.contains((String) tabtpe.table.getValueAt(selection[i], 0))){
								String parcel=tabtpe.table.getValueAt(selection[i], 0).toString().replace("/", "_");
								listrc.add(parcel);
								listrc1.add((String) tabtpe.table.getValueAt(selection[i], 0));
							}
							...
							att = new Thread(){
								public void run(){
									int reponse = JOptionPane.showConfirmDialog(
											null, "Operation termin�e. Voulez-vous ouvrir le fichier Excel ?",
											"Confirmation",
