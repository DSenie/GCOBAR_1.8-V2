...
private Object [] entete={"Code/D�signation Tache","D�signation"};
...
public void composant(final String log){
	 ...
	 list_t = new ArrayList<Object>(Arrays.asList(new String[]{"--S�lectionner un Poste--"}));
     ...
     list_poste_tr=pos.select_poste_code();	  
	   ...
	   // list_zone_tr=use.select_zone_code();
	   list_t = new ArrayList<Object>(Arrays.asList(new String[]{"---S�lectionner une Tache-----"}));
	...
	valid_supp.addActionListener(
           ...
           // use.select_chaine_jtext(designation.getText());
            ...
            if(poste_comb.getSelectedIndex()==0){
	          msg+="Vous Devez d'abord S�lectionner un Poste \n";
	         }
            else if(existe==true){
  	 ...
  	 but_supprim.addActionListener(
             ...
             JOptionPane.showMessageDialog(null, "Vous devez d'abord s�lectionner la tache que vous voulez supprimer", "",
     	 	 JOptionPane.INFORMATION_MESSAGE);
                	                           }
             else{
  	...
  	valid_modif.addActionListener(
		       ...
		       // use.select_chaine_jtext(designation.getText());
		        ...
		        pos.setupdate_poste_tab_delete(poste_fi.getText());
		  		...
		  		if(desc2==true) {msg+="Veuillez remplir la d�signation \n";}
		  		else  if(valid_j==true) { msg+="Un Champ dans le tableau est vide \n";}
		  		else if(repet2==true){ msg+="Il existe une ou plusieurs Taches dupliqu�es \n"; }
		  		
		  		if(!msg.equals("")){JOptionPane.showMessageDialog(null,msg); }
		  		
		  		else{
		  	    JOptionPane.showMessageDialog(null,"Le Poste a �t� bien modifi�");
		  	    designation.setText("");
		  		poste_comb.removeAllItems();
		  		poste_comb.addItem("--S�lectionner une Poste--");
		  		list_poste_tr=pos.select_poste_code();
		  		for(int i=0;i<list_poste_tr.size();i++){
		  		 //Pour affecter une valeur de base de donn�es � un Combobox 
		  		 poste_comb.addItem(list_poste_tr.get(i)+" "+list_poste_tr.get(i+1));
 	       ...
 	       if(existe==false){
 	        ...
 	        if(vaidCheck()==false){valid=true; }
 	    	 if(valid==false)
               ...
               if(repet==false&&desc==false&&valid==false   ) {
                JOptionPane.showMessageDialog(null,"Le Poste a �t� bien ajout�e");
                designation.setText("");
               poste_comb.enable();
			   ...
			   for(int i=0;i<list_poste_tr.size();i++)
			   {
	  		 	...
	  		 	if(desc==true) {msg+="Veuillez remplir la d�signation \n"; }
	  		 	else if(valid==true) { msg+="Un Champ dans le tableau est vide \n";}
	  		 	else if(repet==true){msg+="Il existe plusieurs ou des taches dupliqu�es. \n";}
		  		if(!msg.equals("")){JOptionPane.showMessageDialog(null,msg);}
	  		   }}});
...
if(om.trim().length()==0||om.equals("---S�lectionner une zone-----"))
