...
//	private boolean repet2;
	...
	private Object[] entete = { "Code/Intitul� Tache" ,"article","Temp normal","Marge","Temps Total"};
	...
	public void composant(final String log) {
	    for (String elem : names)
		  ...
		  numero_poste_comb.addItem("--S�lectionner un Numero de Poste--");
		  
		   
			  
			   
			   list_article= new ArrayList<Object>(
						Arrays.asList(new String[] { "---S�lectionner un Article -----" }));
		...
		selectioncomb.selectprocess(profils_comb  ,this,log);	
		  ...
		  list_p = new ArrayList<Object>(Arrays.asList(new String[]{"---S�lectionner une Tache-----"}));
			...
			public void actionPerformed(ActionEvent e) {
				if (tab_observation.table.getSelectedRow() == -1) {
					JOptionPane
							.showMessageDialog(
									null,
									"Vous devez d'abord s�lectionner l'observation que vous voulez supprimer",
									"", JOptionPane.INFORMATION_MESSAGE);
				} else {
			...
			public void actionPerformed(ActionEvent e) {
				if (tab.table.getSelectedRow() == -1) {
					JOptionPane
							.showMessageDialog(
									null,
									"Vous devez d'abord s�lectionner la tache que vous voulez supprimer",
									"", JOptionPane.INFORMATION_MESSAGE);
				} else {
		...
		valid_modif.addActionListener(new ActionListener() {
			...
			//	use.select_zone_jtext(designation.getText());
				...
				if (tab.table.getValueAt(i, 0).toString().equals(tab.table.getValueAt(j, 0).toString())&&
				  ...
				  msg += "Il existe plusieurs ou un tache dupliqu�. \n";
								}
							}
						}}
					if (valid_a == false) {
				...
				if (tab_article.table.getValueAt(i, 0).toString().equals(tab_article.table.getValueAt(j, 0).toString())) {
				  msg += "Il existe plusieurs ou un article dupliqu�. \n";
								}
							}
						}}
					if (valid_o == false) {
				...
				if (tab_observation.table.getValueAt(i, 0).toString().equals(tab_observation.table.getValueAt(j, 0).toString())) {
				  msg += "Il existe plusieurs ou un observation dupliqu�. \n";
								}
							}
						}}
...
//										col2_i, part1,profils_comb.getSelectedItem().toString());
						...
						JOptionPane.showMessageDialog(null,"Les Tache du process a �t� bien modifi�e");
